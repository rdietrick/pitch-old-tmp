 ...
 * A computer player
 */
public class JakeBotCPUPlayer extends CPUPlayer {

	public JakeBotCPUPlayer( User user) {
		super( user);
	}
	...
	/**
	 * (Re-)Initialize state between hands.
	 */
	 ...
	 * Bidding logic.
	 * 
	 * @return the number of points this player bids; 0 for pass.
	 */
	protected int bid() {
		int myBid = Bid.PASS;
	...
	/**
	 * Called every time a player plays a card.
	 * 
	 * If the play doesn't follow suit, this player is noted as having none of
	 * the lead suit.
	 * 
	 * @param index
	...
	/**
	 * Find out if a player possibly has a card in a suit. Only a return value
	 * of false is reliable.
	 */
	...
	/**
	 * Playing logic Decide what card to play, and return it. Look at the
	 * javadocs for following classes to find out what information is available
	 * during the lifetime of this method to aid in making a decision:
	 * 
	 * @see com.pitchplayer.server.Trick,
	 * @see com.pitchplayer.server.Game
	 */
	protected Card play() {
		Trick trick = getTrick(); // see com.pitchplayer.server.Trick
		Card[] hand = getHand(); // get the array of cards in your hand; hand
								 // will only have the unplayed cards
		int trump = trick.getTrump(); // trump will correspond to one of the
									  // suit constants in com.pitchplayer.Card
		...
		if (trick.getPlayCount() == 0) { // playcount indicates the number of
										 // cards already played in the trick
			/*******************************************************************
			 * ******************** MY LEAD **************************
			 ******************************************************************/
			if (trick.getTrickCount() == 0) { // trickcount indicates the number
											  // of tricks already played in the
											  // hand
			...
			if (holdingJack() && willSuitClear(trump)) {
				Card[] trumpCards = getTrump();
	...
	/**
	 * Trump suit will have been proposed in evaluateBid(). If holding a card >
	 * JACK, throw it, otherwise, throw the lowest card in trump suit.
	 */
	...
	/**
	 * Find out if the players after me can beat a particular card. Determines
	 * this based on whether it is known that the players are out of trump and
	 * the lead suit.
	 */
	...
	/**
	 * Called when a chat message is sent from another player. Filters for
	 * profanity and tells the user to watch it, if necessary.
	 * 
	 * @param player
