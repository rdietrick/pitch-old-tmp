 ...
 * A computer player
 */
public class CardCountingCPUPlayer extends CPUPlayer {

	public CardCountingCPUPlayer(User user) {
		super(user);
	}
	...
	protected int evaluateBid(int goingBid) {
		...
		for (int i = 0; i < 4; i++) {
			...
			int totalTrumpValue = 0;
			for (Card c : suitCards) {
				totalTrumpValue += c.getValue();
				// do i have the jack?
				if (c.getValue() == Card.JACK) {
					if (suitCards.length > 4) { // if i have 5 cards in this suit, i can definitely make the jack
						suitPoints[i] += 1;
						holdingJack = true;
					}
					else {
						// estimate the odds of making the jack as: number of trump cards / (number of players +1)
						suitPoints[i] += suitCards.length / ((float)getGame().getNumPlayers()+1);
					}
				}
				else {
					if (c.getValue() > Card.JACK) {
			...
			if (!holdingJack && jackTakers > 0) {
				// calculate the probability of me stealing the jack from someone else
				float jackProb = probCalc.getProbabilityCardsDealt(1, getGame().getHandSize(), getGame().getNumPlayers());
				if (jackTakers > 0) {
					float stealProb = jackProb * (jackTakers/4f);
					suitPoints[i] += stealProb;
				}
			}
			// guesstimate the odds of making game
			// having higher trump cards will give you a better chance.
			// the highest total value of cards in a suit = 57 (Ace - 9) 
			suitPoints[i] += totalTrumpValue / 57;
